...
from pytest_single_file_logging.log_socket_receiver import start_server, configure_client_logger, stop_server
...
def pytest_configure_node(node):
    ip = node.config.server_ip
    node_id = node.config.slaveinput['slaveid']
    l = configure_client_logger(ip, node_id)
...
def logger(request):
    ...
    """
    log = logging.getLogger(request.config.slaveinput['slaveid'])
    return log
